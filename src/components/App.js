...
const App = () => {
 ...
 const handleSearchProductsApi = async (searchWord, currentPage = 1) => {
  ...
  try {
   const { data } = await axios.get(
    `${BASE_URL}?q=${searchWord}&page=${currentPage}&key=${KEY_API}&image_type=photo&orientation=horizontal&per_page=20`
   );
   currentPage === 1
    ? setImages(data.hits)
 ...
 const changeSearchWord = (word) => {
  handleSearchProductsApi(word);
  setSearchWord(searchWord);
 };

 const showMore = async () => {
  handleSearchProductsApi(searchWord, currentPage);
 };
